...
describe('WebSocketService', () => {
  ...
  describe('Workflow Triggering', () => {
    ...
    it('should include stage_models in request payload when provided', async () => {
      const task = {
        id: 123,
        title: 'Test Task',
        description: 'Test description',
      ...
      const triggerPromise = service.triggerWorkflowForTask(task, 'adw_orchestrator', {
        stageModels
      });
      ...
      // Check the message sent via WebSocket
      const sentMessage = mockWs._messageQueue[mockWs._messageQueue.length - 1];
      expect(sentMessage.type).toBe('trigger_workflow');
      expect(sentMessage.data.stage_models).toEqual(stageModels);
    ...
    it('should not include stage_models when not provided in options', async () => {
      const task = {
        id: 123,
        title: 'Test Task',
        description: 'Test description',
      ...
      const sentMessage = mockWs._messageQueue[mockWs._messageQueue.length - 1];
      expect(sentMessage.type).toBe('trigger_workflow');
      expect(sentMessage.data.stage_models).toBeUndefined();

      mockWs.simulateMessage({
        type: 'trigger_response',
        data: { status: 'accepted', adw_id: 'task-456' }
      });
