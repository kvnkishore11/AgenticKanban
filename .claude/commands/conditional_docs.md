...
- app_docs/feature-29aefea6-per-stage-model-selection.md
  - Conditions:
    - When working with AI model configuration or selection
    - When implementing per-stage model settings in workflows
    - When working with WorkflowTriggerModal or StageModelSelector components
    - When troubleshooting model selection UI or model defaults
    - When working with adws/utils/model_config.py or src/utils/modelDefaults.js
    - When implementing stage-specific model overrides in the orchestrator
    - When working with agent execution and model priority logic
    - When adding new AI models to the system
    - When troubleshooting WebSocket stage_models payload integration
    - When working with ADWStateData stage_model_overrides field
    - When implementing cost/performance indicators for models
    - When working with model metadata or display information
