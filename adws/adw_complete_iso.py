...
"""
ADW Complete Iso - AI Developer Workflow for completing and merging to main
...
This workflow extends adw_ship_iso to include worktree cleanup and completion marking.
It should be called when the user wants to finalize and complete an ADW workflow.

Note: Merge operations happen in the main repository root, not in the worktree,
to preserve the worktree's state until the merge is successful.
"""
...
def manual_merge_to_main(branch_name: str, logger: logging.Logger) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "merge", branch_name, "--no-ff", "-m", f"Merge branch '{branch_name}' via ADW Complete workflow"],
            capture_output=True, text=True, cwd=repo_root
        )
        if result.returncode != 0:
            # Try to restore original branch
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run adw_complete_iso.py <issue-number> <adw-id>")
        print("\nError: Both issue-number and adw-id are required")
        print("Run the complete SDLC workflow before completing")
        sys.exit(1)
    ...
    temp_logger = setup_logger(adw_id, "adw_complete_iso")
    state = ADWState.load(adw_id, temp_logger)
    if not state:
        # No existing state found
        logger = setup_logger(adw_id, "adw_complete_iso")
        logger.error(f"No state found for ADW ID: {adw_id}")
        logger.error("Run the complete SDLC workflow before completing")
    ...
    state.append_adw_id("adw_complete_iso")

    # Set up logger with ADW ID
    logger = setup_logger(adw_id, "adw_complete_iso")
    logger.info(f"ADW Complete Iso starting - ID: {adw_id}, Issue: {issue_number}")
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, "ops", "🏁 Starting completion workflow\n"
    ...
    if not is_valid:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_COMPLETER, f"❌ {error_msg}\n\n"
    ...
    if not valid:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_COMPLETER, f"❌ Worktree validation failed: {error}")
        )
        sys.exit(1)
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_COMPLETER, f"📋 State validation complete\n"
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_COMPLETER, f"🔀 Merging {branch_name} to main...\n"
    ...
    if not success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_COMPLETER, f"❌ Failed to merge: {error}")
        )
        sys.exit(1)

    logger.info(f"✅ Successfully merged {branch_name} to main")

    # Step 5: Remove worktree after successful merge
    logger.info(f"Removing worktree at: {worktree_path}")
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_COMPLETER, "🧹 Cleaning up worktree...")
    )

    cleanup_success, cleanup_error = remove_worktree(adw_id, logger)

    if not cleanup_success:
        logger.warning(f"Failed to remove worktree: {cleanup_error}")
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_COMPLETER, f"⚠️ Warning: Failed to remove worktree: {cleanup_error}\n"
                               "You may need to manually clean it up.")
        )
    ...
    state.save("adw_complete_iso")

    # Step 7: Post success message
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_COMPLETER,
                           f"🎉 **Workflow completed!**\n\n"
                           f"✅ Validated all state fields\n"
                           f"✅ Merged branch `{branch_name}` to main\n"
                           f"✅ Pushed to origin/main\n"
                           f"✅ Cleaned up worktree\n"
                           f"✅ Marked workflow as completed\n\n"
                           f"🏁 This ADW workflow is now complete!")
    )

    # Post final state summary
    make_issue_comment(
        issue_number,
        f"{adw_id}_ops: 📋 Final completion state:\n```json\n{json.dumps(state.data, indent=2)}\n```"
    )
