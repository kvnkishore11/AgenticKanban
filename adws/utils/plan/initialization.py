"""ADW initialization utilities for plan workflow."""
...
from adw_modules.kanban_mode import log_mode_status

from .types import InitContext


def parse_cli_arguments(args: List[str]) -> tuple[str, Optional[str]]:
    """Parse command line arguments for plan workflow.
...
def initialize_workflow(
    args: List[str],
    workflow_name: str = "adw_plan_iso"
) -> InitContext:
    """Initialize ADW workflow with environment, state, logger, and notifier.
    ...
    Returns:
        InitContext with all initialized components
    ...
    notifier.notify_start(workflow_name, f"Starting planning workflow for issue #{issue_number}")
